...
_siteConfigReady.then(function() {
  ...
  if (item.instagramReel && item.instagramReel.trim() !== '' &&
    ...
    reelLink.rel = 'noopener noreferrer';
    reelLink.className = 'reel-icon-btn';
    reelLink.title = 'Watch Reel on Instagram';
    reelLink.setAttribute('aria-label', 'Watch ' + item.name + ' reel on Instagram');
    reelLink.addEventListener('click', function(e) { e.stopPropagation(); });
    reelLink.innerHTML =
      ...
      '</div>' +
      '<span class="reel-tooltip">Watch Reel</span>';
    imgWrap.appendChild(reelLink);
  }
    ...
    if (site.journal && site.journal.length > 0) {
      ...
      if (journalGrid) {
        ...
        site.journal.forEach(function(post) {
          ...
          wrap.style.cssText = 'position:relative;width:100%;height:100%;border-radius:8px;overflow:hidden;';
          var img = document.createElement('img');
          img.src = post.image;
          ...
          // Add reel icon if reel URL exists
          if (post.reelUrl && post.reelUrl.trim() !== '' && post.reelUrl.indexOf('instagram.com') !== -1) {
            var reelLink = document.createElement('a');
            reelLink.href = post.reelUrl.trim();
            reelLink.target = '_blank';
            reelLink.rel = 'noopener noreferrer';
            reelLink.className = 'reel-icon-btn';
            reelLink.title = 'Watch Reel on Instagram';
            reelLink.innerHTML =
              '<div class="reel-icon-circle">' +
                ...
                '<div class="reel-play-badge">\u25B6</div>' +
              '</div>';
            wrap.appendChild(reelLink);
          }

          journalGrid.appendChild(wrap);
        });
      }
    }
